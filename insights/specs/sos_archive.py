...
class SosSpecs(Specs):
    alternatives_display_python = simple_file(
        "sos_commands/alternatives/alternatives_--display_python"
    )
    ...
    blkid = first_file(
        ["sos_commands/block/blkid_-c_.dev.null", "sos_commands/filesys/blkid_-c_.dev.null"]
    )
    ...
    chkconfig = first_file(
        ["sos_commands/startup/chkconfig_--list", "sos_commands/services/chkconfig_--list"]
    )
    ...
    crictl_logs = glob_file(
        [
            "sos_commands/crio/containers/crictl_logs_-t*",
            "sos_commands/crio/containers/logs/crictl_logs_-t*",
        ]
    )
    ...
    date = first_of(
        [simple_file("sos_commands/general/date"), simple_file("sos_commands/date/date")]
    )
    ...
    foreman_production_log = first_of(
        [
            ...
            simple_file("sos_commands/foreman/foreman-debug/var/log/foreman/production.log"),
        ]
    )
    foreman_proxy_conf = first_of(
        [
            simple_file("/etc/foreman-proxy/settings.yml"),
            simple_file("sos_commands/foreman/foreman-debug/etc/foreman-proxy/settings.yml"),
        ]
    )
    foreman_proxy_log = first_of(
        [
            simple_file("/var/log/foreman-proxy/proxy.log"),
            simple_file("sos_commands/foreman/foreman-debug/var/log/foreman-proxy/proxy.log"),
        ]
    )
    ...
    gnocchi_metricd_log = first_file(
        ["/var/log/containers/gnocchi/gnocchi-metricd.log", "/var/log/gnocchi/metricd.log"]
    )
    grubenv = first_file(["/boot/grub2/grubenv", "/boot/efi/EFI/redhat/grubenv"])
    hammer_ping = first_file(
        ["sos_commands/foreman/hammer_ping", "sos_commands/foreman/foreman-debug/hammer-ping"]
    )
    ...
    hostname_default = first_file(
        ["sos_commands/general/hostname", "sos_commands/host/hostname", "/etc/hostname", "hostname"]
    )
    ...
    ip_neigh_show = first_file(
        ["sos_commands/networking/ip_-s_-s_neigh_show", "sos_commands/networking/ip_neigh_show"]
    )
    ...
    ls_dev = first_file(
        [
            ...
            "sos_commands/block/ls_-alZR_.dev",
            "sos_commands/devicemapper/ls_-alZR_.dev",
        ]
    )
    ls_sys_firmware = first_file(
        ["sos_commands/boot/ls_-lanR_.sys.firmware", "sos_commands/boot/ls_-alZR_.sys.firmware"]
    )
    ...
    mongod_conf = glob_file(["/etc/mongod.conf", "/etc/opt/rh/rh-mongodb34/mongod.conf"])
    ...
    multipath__v4__ll = first_file(
        ["sos_commands/multipath/multipath_-v4_-ll", "sos_commands/devicemapper/multipath_-v4_-ll"]
    )
    ...
    neutron_conf = first_file(
        [
            "/var/lib/config-data/puppet-generated/neutron/etc/neutron/neutron.conf",
            "/etc/neutron/neutron.conf",
        ]
    )
    neutron_dhcp_agent_ini = first_file(
        [
            "/var/lib/config-data/puppet-generated/neutron/etc/neutron/dhcp_agent.ini",
            "/etc/neutron/dhcp_agent.ini",
        ]
    )
    neutron_l3_agent_ini = first_file(
        [
            "/var/lib/config-data/puppet-generated/neutron/etc/neutron/l3_agent.ini",
            "/etc/neutron/l3_agent.ini",
        ]
    )
    neutron_l3_agent_log = first_file(
        ["/var/log/containers/neutron/l3-agent.log", "/var/log/neutron/l3-agent.log"]
    )
    ...
    neutron_ovs_agent_log = first_file(
        [
            "/var/log/containers/neutron/openvswitch-agent.log",
            "/var/log/neutron/openvswitch-agent.log",
        ]
    )
    neutron_plugin_ini = first_file(
        [
            "/var/lib/config-data/puppet-generated/neutron/etc/neutron/plugin.ini",
    ...
    nmcli_dev_show_sos = glob_file(
        ["sos_commands/networking/nmcli_dev_show_*", "sos_commands/networkmanager/nmcli_dev_show_*"]
    )
    ...
    openvswitch_daemon_log = first_file(
        ['/var/log/openvswitch/ovs-vswitchd.log', '/host/var/log/openvswitch/ovs-vswitchd.log']
    )
    ...
    pcs_status = first_file(
        ["sos_commands/pacemaker/pcs_status", "/sos_commands/pacemaker/pcs_status_--full"]
    )
    ...
    rhn_hibernate_conf = first_file(
        ["/usr/share/rhn/config-defaults/rhn_hibernate.conf", "/config-defaults/rhn_hibernate.conf"]
    )
    rhn_schema_version = simple_file("sos_commands/satellite/rhn-schema-version")
    rhn_search_daemon_log = first_file(
        ["/var/log/rhn/search/rhn_search_daemon.log", "/rhn-logs/rhn/search/rhn_search_daemon.log"]
    )
    ...
    rhn_taskomatic_daemon_log = first_file(
        ["/var/log/rhn/rhn_taskomatic_daemon.log", "rhn-logs/rhn/rhn_taskomatic_daemon.log"]
    )
    ...
    subscription_manager_status = simple_file(
    ...
    swift_object_expirer_conf = first_file(
        [
            "/var/lib/config-data/puppet-generated/swift/etc/swift/object-expirer.conf",
    ...
    uptime = first_of(
        [simple_file("sos_commands/general/uptime"), simple_file("sos_commands/host/uptime")]
    )
